...
setting(endId).title="";
setting(endId).fileName="siiPred_refCond_mandGendSpecBIF.";
setting(endId).genspec=true;
setting(endId).nu=22;
setting(endId).flag="Mandarin gender-specific";
setting(endId).legendOn=false;
setting(endId).refSii=getRefSIIMeasurement(setting(endId).refshift,dataPath );
...
setting(endId).refshift=0;
setting(endId).title="";
setting(endId).fileName="siiPred_refeCond_mandAverageBIF.";
...
setting(endId).refshift=0;
setting(endId).title="";
setting(endId).fileName="siiPred_refCond_nnsBIF.";
...
setting(endId).refshift=0;
setting(endId).title="";
setting(endId).fileName="siiPred_refCond_stadardTable3.";
...
%bif optimized for bad performance
endId=length(setting)+1;
setting(endId).refshift=0;
setting(endId).title="";
setting(endId).fileName="siiPred_refCond_bad3.";
setting(endId).bif=bifDat.bad_refcond;
setting(endId).genspec=false;
setting(endId).nu=21;
setting(endId).flag="bad_refcond";
setting(endId).legendOn=true;
setting(endId).refSii=getRefSIIMeasurement(setting(endId).refshift,dataPath,setting(endId).bif );
...
%save the data to work with it

%save(setting(id).flag+"refSII"+".mat","x","xe","y")
